...
MFAMDH264Encoder::MFAMDH264Encoder(MFPipeline* pipeline, const VFVideoMediaType sourceMediaType, const VFMFVideoEncoderSettings settings) :
...
MFAMDH264Encoder::~MFAMDH264Encoder()
{
	//encoderCb->Release();
...
HRESULT MFAMDH264Encoder::Start()
{
	if (!Initiated)
	{
		return E_FAIL;
...
HRESULT MFAMDH264Encoder::ForceKeyFrame() const
{
	if (codecAPI)
	{
...
HRESULT MFAMDH264Encoder::ApplySettings()
{
...
bool MFAMDH264Encoder::Init()
{
	...
	hr = codecs.GetAMDH264Encoder(&codecActivate);

	if (FAILED(hr))
	{
		TraceE(L"AMD H.264 encoder not found.\n");
		return false;
	}
	...
	if (FAILED(hr))
	{
		TraceE(L"AMD H.264 encoder Unable to activate encoder.\n");
		return hr;
	}
...
MFAMDH264Encoder::EncoderEventCallback::EncoderEventCallback(MFPipeline *pipeline, MFAMDH264Encoder *pEncoder)
{
	Pipeline = pipeline;
...
MFAMDH264Encoder::EncoderEventCallback::~EncoderEventCallback()
{

}

STDMETHODIMP MFAMDH264Encoder::EncoderEventCallback::QueryInterface(REFIID _riid, void** pp_v)
{
...
STDMETHODIMP_(ULONG) MFAMDH264Encoder::EncoderEventCallback::AddRef()
{
	return InterlockedIncrement(&ref_count);
}

STDMETHODIMP_(ULONG) MFAMDH264Encoder::EncoderEventCallback::Release()
{
...
STDMETHODIMP MFAMDH264Encoder::EncoderEventCallback::GetParameters(DWORD *p_flags, DWORD *p_queue)
{
	return E_NOTIMPL;
}

LONGLONG MFAMDH264Encoder::CurrentPosition()
{
	return encoderCb->CurrentPosition();
}

LONGLONG MFAMDH264Encoder::EncoderEventCallback::CurrentPosition()
{
	return _lastTimestamp;
}

STDMETHODIMP MFAMDH264Encoder::EncoderEventCallback::Invoke(IMFAsyncResult *pAsyncResult)
{
